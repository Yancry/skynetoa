...
/**
 * 
 * 类名：MyHttpSessionListener 功能：session监听器 详细： 作者：Wangyan 版本：1.0 日期：2017-7-11
 * 上午8:46:26
 *
 */
public class MyHttpSessionListener implements HttpSessionListener {
	...
	public void sessionCreated(HttpSessionEvent e) {
		...
		// 获取全局在线用户
		Map<String, OnLineUser> usersMap = (Map<String, OnLineUser>) session.getServletContext()
		...
		if (me != null) {
			...
			if (ol != null) {
				...
				if (session.getAttribute("isOut") != null) {
					...
					log.setLoginDesc(
							"正常退出(" + DateUtil.timestamp2String(DateUtil.currentTimestamp(), "MM-dd HH:mm:ss") + ")");

				} else {

					log.setLoginDesc(
							"非正常退出(" + DateUtil.timestamp2String(DateUtil.currentTimestamp(), "MM-dd HH:mm:ss") + ")");
				}
			...
			System.out
					.println("用户:" + MyCache.getInstance().getTrueName(me.getId()) + "下线之后，用户在线数量:" + usersMap.size());
