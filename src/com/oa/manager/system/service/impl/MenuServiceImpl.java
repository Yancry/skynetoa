...
public class MenuServiceImpl extends BaseServiceImpl implements IMenuService {
	...
	public List<Map<String, Object>> selectMenusByUserId(String userId) {
		...
		StringBuffer hql = new StringBuffer(
				" select distinct new Map(" + "m.id as id,m.menuName as name,m.menuSuperId as pid,m.menuUrl as url,"
		...
		if (ServletUtil.isDeveloper()) {
			...
			hql.append(
					" SyRoleMenu rm, SyMenu m where rm.menuId=m.id and rm.roleId in(:roleIds)  and  m.menuStatus=1 ");
	 ...
	 * List<Map<String,Object>> selectMyMenusfor(){
	 * 
	 * List<String> roleIds=ServletUtil.getRoleIdsByUserId(userId); StringBuffer
	 * hql=new StringBuffer(" select distinct new Map(" +
	 * "m.id as id,m.menuName as name,m.menuSuperId as pid,m.menuUrl as url," +
	 * "m.menuExternal as external,m.menuFresh as fresh,m.menuIcon as icon,m.menuOpen as open,"
	 * + "m.menuRel as rel,m.menuTarget as target )from "); Map map=new
	 * HashMap(); if(ServletUtil.isDeveloper()){ hql.append(" SyMenu m ");
	 * }else{ hql.
	 * append(" SyRoleMenu rm, SyMenu m where rm.menuId=m.id and rm.roleId in(:roleIds)  and  m.menuStatus=1 "
	 * ); map.put("roleIds", roleIds); } return
	 * dao.find(hql+" order by m.menuSort asc ",map);
	 * 
	 * }
	 */
	...
	public List<Map<String, Object>> queryMenus() {
		if (ServletUtil.isDeveloper()) {
			return dao.find(
					"select new Map(m.id as id,m.menuName as menuName,m.menuSuperId as menuSuperId,m.menuIcon as menuIcon,m.menuOpen as menuOpen)from SyMenu m order by menuSort asc");
		} else {
			return dao.find(
					"select new Map(m.id as id,m.menuName as menuName,m.menuSuperId as menuSuperId,m.menuIcon as menuIcon,m.menuOpen as menuOpen) from SyMenu m where menuStatus=1 order by menuSort asc");
		}
	}
	...
	public String updateMenu(SyMenu m) {
		...
		Object obj = dao.findOne("from SyMenu where menuName=? and menuSuperId=? and id!=?", m.getMenuName(),
				m.getMenuSuperId(), m.getId());
		if (obj == null) {
	...
	public Map findMenuById(String id) {
		return (Map) dao.findOne(
				"select new Map(m as m,(select menuName from SyMenu where id=m.menuSuperId)as superName)from SyMenu m where m.id=?",
				id);
	...
	public String addSyMenuMy(SyMenuMy m) {
		...
		Object obj = dao.findOne("from SyMenuMy  where userId=? and menuId=? ", ServletUtil.getMember().getId(),
				m.getMenuId());
		if (obj == null) {
	...
	public String updateSyMenuMy(SyMenuMy m) {
		...
		Object obj = dao.findOne("from SyMenuMy where userId=? and menuId=? and id!=? ",
				ServletUtil.getMember().getId(), m.getMenuId(), m.getId());
		if (obj == null) {
	...
	public List selectMySyMenuMy() {
		...
		return dao.find(
				" select new Map(m.menuName as name,m.menuExternal as external,m.menuIcon as icon,m.menuRel as rel,m.menuTarget as target,m.menuUrl as url) from  SyMenu m ,SyMenuMy my where  my.menuId=m.id and my.userId=?  and  m.menuStatus=1  order by my.sort asc ",
				ServletUtil.getMember().getId());
	}
