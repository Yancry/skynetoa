package com.oa.manager.personnel.service.impl;
...
import com.oa.manager.personnel.bean.XtAttendanceTimeUser;
...
public class AttendanceServiceImpl extends BaseServiceImpl implements
	...
	public DataGrid selectSignStatistics(PageParam param,String userIds,String deptId, Date startDate,Date endDate){
		...
		for(Map<String,Object> map:rows){
			...
			//获取正常考勤登记数量 
			map.put("normalCount",(Long)dao.findUniqueOne("select count(islate)   from XtAttendanceSign s where signuser = '"+userid+"' and islate='0' "+sb,list));
			//迟到登记数量 
			map.put("lateCount",(Long)dao.findUniqueOne("select count(islate)   from XtAttendanceSign s where signuser = '"+userid+"' and islate='1' "+sb,list));
			//早退登记数量
			map.put("earlyCount",(Long)dao.findUniqueOne("select count(islate)   from XtAttendanceSign s where signuser = '"+userid+"' and islate='2' "+sb,list));
			//请假登记数量
			map.put("leaveCount",(Long)dao.findUniqueOne("select count(islate)   from XtAttendanceSign s where signuser = '"+userid+"' and islate='3' "+sb,list));
			//旷工登记数量
			map.put("absenteeismCount",(Long)dao.findUniqueOne("select count(islate)   from XtAttendanceSign s where signuser = '"+userid+"' and islate='4' "+sb,list));
			
			//外出登记数量
			map.put("outCount",(Long)dao.findUniqueOne("select count(islate)   from XtAttendanceSign s where signuser = '"+userid+"' and islate='5' "+sb,list));
			//其他
			map.put("otherCount",(Long)dao.findUniqueOne("select count(islate)   from XtAttendanceSign s where signuser = '"+userid+"' and islate='6' "+sb,list));
			//查询时间段
			map.put("times", Month);
			//未登记数量  计算两个日期之间的天数 -
		}
