	...
	<form  onsubmit="return datagridSearch(this,'${param.rel }_datagrid');"  >
		...
		<div class="search-toolbar" >
			<span style="float:left;">
				<shiro:hasPermission name="task:addCandidate">
					<a class="easyui-linkbutton"  icon="icon-edit"	plain="true"
					  title="追加任务候选人"	href="task/todo/notHave/entrustPage.do?taskId={id}" target="dialog" width="600" height="300" rel="${param.rel}_addhxr" warn="请先选择一个任务" >追加候选人</a>
				</shiro:hasPermission>
			</span>
...
<!--	
	...
	$(function() {
		...
		$('#<%=request.getParameter("rel")%>_datagrid').datagrid({
			nowrap:false,
			pagination:false,
			url : "task/todo/notHave/query.do",
			columns : [ [ 
				...
				},
				{
					...
					formatter: function(value,row,index){
						...
						return '<a href="task/receive.do?taskId='+row.id+'" target="ajaxTodo" datagrid="<%=request.getParameter("rel")%>_datagrid"  title="确认要签收任务吗">签收</a>'
						       +' | <a href="'+row.completeUrl+'?taskId='+row.id+'&rel=<%=request.getParameter("rel")%>" target="dialog" width="1000" height="450"  title="任务办理">办理</a>';
					}
				}
