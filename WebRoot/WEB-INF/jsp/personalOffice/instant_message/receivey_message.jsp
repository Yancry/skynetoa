	...
	<form  onsubmit="return datagridSearch(this,'${param.rel }_datagrid');" >
		<div  class=" search-content " >
			...
			<span>
				...
				<input type="hidden"  name="sendUid"  toName="user.id"  />
...
<!--	
	...
	$(function() {
		...
		$('#<%=request.getParameter("rel")%>_datagrid').datagrid({
			url : "instantMessage/selectReceiveMessage/query.do",
			columns : [ [ 
			    {
			    	field:"ck",
			    	title : "勾选",
				...
				},
				{
					...
					formatter: function(value,row,index){
						...
						return ' <a href="instantMessage/addPage.do?userId='+row.sendUid+'&rel=<%=request.getParameter("rel")%>" target="dialog" width="1000" height="450" rel="<%=request.getParameter("rel")%>_add" title="发送即时消息">'+value+'</a>';
					}
				...
				},
				{
					...
					width : 350,
					formatter: function(value,row,index){
						return '<a href="instantMessage/show.do?id='+row.id+'&rel=<%=request.getParameter("rel")%>_show&type=1" target="dialog" width="800" height="400" title="即时沟通详情" rel="<%=request.getParameter("rel")%>_show">'+value+'</a>';
					}
