	...
	<form action="email/addDraft.do"  enctype="multipart/form-data" method="post" 
		...
		<table class="table table-bordered " >	
			<tr>
				...
				<td>
					...
					<textarea  readonly="readonly" class="easyui-validatebox" required="true"
						toName="user.trueName" style="width: 210PX" rows="2" ><my:outTrueName id="${ed.edInUid}"/></textarea>
					
					</a>
					<input type="hidden"  name="userIds" toName="user.id" value="${ed.edInUid }" />
			...
			<tr>
				...
				<td>
					<div>
						...
						<tbody>
							<c:forEach  var="ef"  items="${requestScope.efList }" varStatus="status">
								<tr >	
			...
			<tr>
				<th>内容：</th>
				<td>
				<textarea  name="eoDetail"  rows="15" class="editor" style="width: 98%;"><c:out value="${ed.edDetail}" /></textarea>
				</td>
			</tr>
...
function mailboxaddcaogao(bt){
	var $form=$(bt).closest("form");
	$form.attr("action","email/update.do");
