		...
		<table class="table table-bordered " >	
			<tr>
				...
				<a href="user/lookUpPage.do?type=2" 	lookupGroup="user" title="用户查询">
					...
					<input type="hidden"  class="easyui-validatebox" required="true"  name="userIds" toName="user.id" />
			...
			<tr>
				<th>标题：</th>
				<td>
					<input type="text" name="eoName" class="easyui-validatebox" required="true"  maxlength="50" value="转发：<c:out value="${email.emInbox.eiName }"/>"  size="80" />
				</td>
			</tr>
			...
			<tr>
				<th>内容：</th>
				<td>
				<textarea  name="eoDetail"  rows="15" class="editor" style="width: 98%;"><c:out value="${email.emInbox.eiDetail }"/></textarea>
				</td>
			</tr>
			<input type="hidden" name="type" value="in"/>
			<input type="hidden" name="id" value="${email.emInbox.id}"/>
			<input  type="hidden"  name="getId" value="${email.emInbox.id}"/>
...
function mailboxaddfasong(bt){
	...
	$form.attr("action","email/addIn.do");
