			...
			<span style="float:left;">
				...
				<shiro:hasPermission name="shareFiles:addFile">
					<a class="easyui-linkbutton"  icon="icon-add"	plain="true"
						 href="shareFiles/addFilePage.do?rel=${param.rel }&id={#${param.rel }_tree_wdSuperId}" target="dialog" width="650" height="360"  rel="${param.rel }_addfile"  title="创建文件">创建文件</a>
				</shiro:hasPermission>
				<shiro:hasPermission name="shareFiles:updateFile">
...
<!--	
	...
	$(function() {
		...
		$('#<%=request.getParameter("rel")%>_datagrid').datagrid({
			url : "shareFiles/query.do?wdType=1",
			nowrap:false,
			queryParams: {
			...
			columns : [ [ 
				...
				},
				{
					...
					formatter: function(value,row,index){
						...
						return '<a href="shareFiles/download.do?newName='+row.wdNewName+'&name='+encodeURIComponent(row.wdName)+'.'+row.wdExt+'&ext='+row.wdExt+'" target="_blank"  title="点击下载">下载</a>';
